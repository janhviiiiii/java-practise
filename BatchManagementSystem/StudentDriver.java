...
public class StudentDriver {
    // static because: to maintain single copy, no obj creation, global
        ...
        do {
            if (capacity == 0) {
                ...
                if (newInput == 'y') {
                    ...
                    capacity = batch.length - pointer;   //calculating the remaining available slots //6-3 = 3 slot remaining
                } else {
                    break;
                }
            }
            ...
            batch[pointer++] = student; // to place student batch[0]->batch[1]->batch[2]
            capacity--; // decrease capacity by 1 -> 3-> 2-> 1-> 0
            System.out.println("Student added successfully!\n");
        } while (true);

        System.out.println("\n--- STUDENT BATCH DETAILS ---");  // Print the batch information
        for (int i = 0; i < batch.length; i++) {
            if (batch[i] == null) { // to avoid null pointer exception
                continue;
            }
